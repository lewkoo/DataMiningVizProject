...
void Level::calculateItemsetLocations()
{
	...
	this->circle_radius = 50 * levelId * RADIUS_EXPANSION_FACTOR;
...
void Level::drawItemsets()
{
	...
	for(int i = 0; i < elements.size(); i++)
	{
		...
		if(currentItemset->getIsCurrentlySelected() == true)
		{
            ...
            ofSpherePrimitive sphere = ofSpherePrimitive(currentItemset->getRadius()+SELECTED_SPHERE_RADIUS_EXPANSION_FACTOR, 10);
            sphere.setPosition(currentItemset->getLocation().x, currentItemset->getLocation().y, currentItemset->getLocation().z);
            sphere.draw();
            
			//ofSpherePrimitive(currentItemset->getLocation(),currentItemset->getRadius()+SELECTED_SPHERE_RADIUS_EXPANSION_FACTOR);
		...
		}else
		{
            ...
            ofSpherePrimitive sphere = ofSpherePrimitive(currentItemset->getRadius(), 10);
            sphere.setPosition(currentItemset->getLocation().x, currentItemset->getLocation().y, currentItemset->getLocation().z);
            sphere.draw();
            
			//ofSpherePrimitive(currentItemset->getLocation(), currentItemset->getRadius());
			ofNoFill();
		}
